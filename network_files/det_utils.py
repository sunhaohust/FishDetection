...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_idxs):
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            pos_idx_per_image_mask[pos_idx_per_image] = torch.tensor(1, dtype=torch.uint8).cuda()
            neg_idx_per_image_mask[neg_idx_per_image] = torch.tensor(1, dtype=torch.uint8).cuda()
...
class Matcher(object):
    ...
    def __call__(self, match_quality_matrix):
        ...
        matches[below_low_threshold] = torch.tensor(self.BELOW_LOW_THRESHOLD).cuda()  # -1

        # iou在[low_threshold, high_threshold]之间的matches索引置为-2
        matches[between_thresholds] = torch.tensor(self.BETWEEN_THRESHOLDS).cuda()   # -2
